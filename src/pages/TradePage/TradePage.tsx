...
export const TradeChart = ({ pool, assetIds, spotPrice }: TradeChartProps) => {
  ...
  useEffect(() => {
    if (!historicalBalancesData?.historicalBalances || !math || !spotPrice) return;
    const dataset = historicalBalancesData
      .historicalBalances
      .map(({ createdAt, assetABalance, assetBBalance }) => {
        return {
          x: createdAt,
          y: (() => {
            const assetOutLiquidity = assetIds.assetOut === historicalBalancesData.XYKPool.assetAId
              ? assetABalance
              : assetBBalance
            
            const assetInLiquidity = assetIds.assetIn === historicalBalancesData.XYKPool.assetAId
              ? assetABalance
              : assetBBalance
