...
export const generateMiddleTicks = (firstTick: number, lastTick: number, numberOfTicks: number) => {
    const dateRange = lastTick - firstTick;
    const distance = dateRange / (numberOfTicks + 1);
    return times(numberOfTicks)
        .map((_, i) => {
...
export const ChartTicks = ({
    ...
    const ticks = useMemo(() => {
        ...
        let ticks = [firstTick];
        const middleTicks = generateMiddleTicks(firstTick, lastTick, numberOfMiddleTicks);
